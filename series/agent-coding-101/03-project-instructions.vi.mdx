...
description: "Cách viết AGENTS.md hoặc CLAUDE.md vừa đủ để agent hiểu repo, giữ đúng quy ước và không bắt mình kể lại từ đầu."
author: howznguyen
...
Nhưng sang session tiếp theo, mình lại gặp một vấn đề khác. Agent không còn nhớ project dùng `pnpm` hay `npm`. Không biết folder nào chứa business logic. Không biết team không cho phép sửa <TermNote id="database-migration">migration</TermNote> cũ. Mấy điều mình đã giải thích hôm qua, hôm nay phải nói lại từ đầu.
...
- <TermNote id="secret">Secret</TermNote>, token, password hoặc credential
