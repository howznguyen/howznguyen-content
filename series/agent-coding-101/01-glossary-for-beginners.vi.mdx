...
Context window. Token. System prompt. Hallucination. Memory. CLAUDE.md. AGENTS.md. MCP. Agentic loop.
...
**LLM** là viết tắt của Large Language Model. Đây là bộ não đằng sau mọi AI coding tool bạn đang dùng. ChatGPT dùng GPT. Claude Code dùng Claude. Cursor, Copilot - đều chạy trên một LLM nào đó.

Và mình thấy cách dễ nhất để hiểu mọi thuật ngữ trong bài này là nghĩ về bộ não con người. Vì LLM thực sự được thiết kế lấy cảm hứng từ cách não hoạt động - neural network, pattern recognition, prediction.
...
Nếu LLM là bộ não, thì **Agent** là bộ não có thêm tay, chân, và mắt.
...
**Prompt** là bất cứ thứ gì bạn gửi cho AI. Câu hỏi, yêu cầu, mô tả, instruction - tất cả đều là prompt.
...
- **Instruction**: bạn muốn AI làm gì
- **Context**: thông tin nền để AI hiểu bối cảnh
- **Constraint**: giới hạn, ràng buộc, điều kiện
...
Con người đọc theo từ. Nhưng bộ não AI không đọc theo từ - nó đọc theo **token**.
...
**Context window** là tổng dung lượng mà AI có thể "nhìn thấy" trong một lần xử lý. Bao gồm tất cả: system prompt, lịch sử hội thoại, file đang đọc, code đang phân tích, và cả câu trả lời nó đang viết.
...
Context window cũng vậy, nhưng đo bằng token thay vì "7 thứ". Mọi thứ bạn nói, mọi file agent đọc, mọi kết quả command - tất cả đều chiếm chỗ trong bộ nhớ ngắn hạn này. Khi đầy, những thứ cũ phải bị xóa hoặc nén lại.
...
Hiện tại, các model mới có context window khá lớn (100K-200K token). Nhưng lớn không có nghĩa là vô hạn. Và context dài không đồng nghĩa với hiểu sâu - có hiện tượng gọi là "lost in the middle", giống khi bạn đọc một email quá dài và chỉ nhớ đoạn đầu với đoạn cuối.

## System Prompt - tiềm thức của bộ não

**System prompt** là một đoạn instruction đặc biệt mà AI luôn đọc trước khi xử lý bất cứ thứ gì bạn nói.
...
Trong agent coding, system prompt thường chứa: personality của AI, rules cần tuân theo, cách format output, và các constraint an toàn. Khi bạn dùng Claude Code hay Cursor, chúng đều có system prompt riêng mà bạn không nhìn thấy trực tiếp.
...
**Memory** là các cơ chế giúp agent có "bộ nhớ dài hạn" - nhớ xuyên suốt nhiều cuộc trò chuyện.

Con người có nhiều loại bộ nhớ: ngắn hạn (đang nghĩ gì), dài hạn (kinh nghiệm tích lũy), thủ tục (kỹ năng đã thành phản xạ). Agent cũng có nhiều dạng memory tương tự:

**Conversation history** - bộ nhớ ngắn hạn. Lịch sử chat trong cùng một session. Bị giới hạn bởi context window.

**Persistent memory** - bộ nhớ dài hạn. Thông tin được lưu lại giữa các session. Ví dụ: Claude Code có thể lưu memory về preference của bạn, convention của project, decision đã make. Lần sau mở lại, nó vẫn nhớ.

**Project files** - kiến thức nền. Các file như `CLAUDE.md`, `AGENTS.md` đóng vai trò như "kiến thức chuyên môn" mà agent đọc mỗi khi bắt đầu làm việc. Giống developer đọc wiki nội bộ trước khi bắt tay vào project mới.
...
**Hallucination** là khi AI tạo ra thông tin không có thật, nhưng trình bày nó một cách rất tự tin.
...
LLM cũng hoạt động tương tự. Nó không "biết" đúng sai. Nó dự đoán token tiếp theo dựa trên pattern đã học. Khi gặp câu hỏi mà training data không cover rõ, nó không dừng lại nói "tôi không biết". Nó fill vào chỗ trống bằng thứ nghe hợp lý nhất - giống hệt cách não bạn fill ký ức giả vào chỗ bạn không nhớ.
...
**MCP (Model Context Protocol)** - một chuẩn giao tiếp giúp agent kết nối với tool bên ngoài. Ví dụ: đọc database, gọi API, truy cập file system. Giống như USB cho AI - cắm vào là dùng được.

**Agentic loop** - vòng lặp cốt lõi của agent: suy nghĩ → hành động → quan sát kết quả → suy nghĩ tiếp. Agent không làm một lần rồi xong. Nó lặp đi lặp lại cho tới khi task hoàn thành (hoặc bí).

**RAG (Retrieval-Augmented Generation)** - kỹ thuật cho agent search thông tin trước khi trả lời, thay vì chỉ dựa vào kiến thức đã học. Giống tra Google trước khi trả lời câu hỏi, thay vì cố nhớ.

**Compaction** - khi context window gần đầy, hệ thống tự động nén lịch sử hội thoại lại. Giữ ý chính, bỏ chi tiết. Giống ghi chú tóm tắt thay vì giữ nguyên bản gốc.
