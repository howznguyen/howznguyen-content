...
Hồi mới dùng coding agent, mình bị ngợp bởi một đống từ.
...
Đây không phải kiểu bảng thuật ngữ A-Z, đọc được ba dòng là mắt bắt đầu tự đóng.
...
Mình sẽ dùng một hình ảnh xuyên suốt: **bộ não con người**. Nó không hoàn hảo về mặt khoa học, nhưng đủ dễ hình dung để mình không phải mở thêm năm tab chỉ để hiểu một từ.
...
Nên xuyên suốt bài này, mình sẽ dùng bộ não làm hình ảnh so sánh chính.
...
Nhưng có một điểm khác biệt lớn: nó không có ký ức dài hạn mặc định. Mỗi lần bạn mở một cuộc trò chuyện mới, nó gần như bắt đầu từ số 0. Không biết bạn là ai. Không biết project đang làm gì. Cũng không nhớ hôm qua hai bên vừa tranh luận chuyện đặt tên biến ra sao.
...
Ví dụ: "Sửa bug trong auth redirect. File ở src/auth/redirect.ts. Giữ style hiện tại, đừng thêm <TermNote id="abstraction">abstraction</TermNote> mới."
...
Trong agent coding, system prompt thường chứa: tính cách của AI, quy tắc cần tuân theo, cách trình bày đầu ra và các ràng buộc an toàn. Khi bạn dùng Claude Code hay Cursor, chúng đều có system prompt riêng mà mình không nhìn thấy trực tiếp.
...
**Persistent memory** (bộ nhớ lâu dài) - thông tin được lưu lại giữa các session. Ví dụ: agent có thể nhớ sở thích của bạn, quy ước của project hoặc quyết định đã chốt. Lần sau mở lại, mình đỡ phải kể lại từ chương một.
...
Thực ra não người cũng có lúc tự điền vào chỗ trống. Khi ai đó hỏi một chuyện mình không nhớ rõ, não vẫn có thể lắp ghép vài mảnh ký ức rồi đưa ra câu trả lời nghe rất hợp lý, dù chưa chắc đúng. Không hẳn cố bịa, chỉ là tự tin hơi sớm.

LLM cũng hoạt động gần như vậy. Nó dự đoán token tiếp theo từ những mẫu đã học. Khi dữ liệu huấn luyện không đủ rõ, nó vẫn có thể điền vào chỗ trống bằng câu trả lời nghe hợp lý nhất. Vấn đề là câu sai vẫn được viết trôi chảy như câu đúng.
...
- Tự tin giải thích một cách hoạt động sai
...
Nội dung thường giống `CLAUDE.md`: convention, giới hạn và cách làm chung. Nhưng mình viết sao cho không phụ thuộc vào đúng một tool hay tự đoán agent nào đang đọc.
