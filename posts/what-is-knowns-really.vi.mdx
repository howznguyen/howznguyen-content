...
Nếu task của bạn là restructure thư mục, đổi naming, gom file, tách module, cleanup một đoạn code rõ scope, thì agent có thể tự đọc <TermNote id="repository">repo</TermNote> và làm được. Nó dùng `rg`, đọc import, xem file liên quan, rồi sửa.

Với những task mà source of truth đã nằm hết trong codebase, mình không nghĩ Knowns nên chen vào.

Không phải việc gì cũng cần task formal. Không phải refactor nào cũng cần doc. Không phải sửa bug nào cũng cần <TermNote id="memory">memory</TermNote>.
...
- <TermNote id="acceptance-criteria">acceptance criteria</TermNote> không nằm trong code
...
Knowns không phải phép màu. Nó cũng không thay bạn suy nghĩ. Nó chỉ là một CLI tool <TermNote explanation="Local-first nghĩa là công cụ ưu tiên dữ liệu và file nằm trên máy của bạn, thay vì biến một dịch vụ từ xa thành nguồn duy nhất nắm mọi thứ.">local-first</TermNote> để giúp bạn tổ chức task, docs, memory, template và workflow theo cách agent có thể retrieve được.
...
Task đó có thể có mô tả, acceptance criteria, notes, link tới docs hoặc <TermNote id="spec">spec</TermNote>.
