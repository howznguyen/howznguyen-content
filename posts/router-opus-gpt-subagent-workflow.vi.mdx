...
<Callout type="note">
  Đây không phải "cách đúng duy nhất". Đây là workflow mình đang dùng và thấy hợp với
  kiểu làm feature lớn, nhiều bước, cần giữ context và kiểm soát chi phí.
</Callout>
...
<Quote color="blue">
  Không phải model nào cũng nên làm mọi việc. Model mạnh nhất nên giữ vai trò cần
  suy luận mạnh nhất.
</Quote>
...
<TutorialSteps autoAdvance={5000}>
  ...
  <Step
    ...
    height={1169}
  >
    <Highlight x={470} y={470} w={560} h={240} label="Khu vực cấu hình env (BASE_URL, AUTH_TOKEN, model routes)" />
  </Step>
  <Step
    image="https://res.cloudinary.com/dkxhoyenc/image/upload/v1779196339/SCR-20260519-rnzv_ryekyo.png"
    width={1800}
  ...
  >
    ...
    <Highlight x={810} y={770} label="Hoặc Manual Config nếu chạy trên server" />
  </Step>
</TutorialSteps>
...
<Callout type="warning">
  Đừng paste API key vào blog, repo, issue, hay prompt public. Config local nên để
  trong env var hoặc file local đã được gitignore.
</Callout>
...
<Callout type="info">
  Đây là điểm hay: Claude Code có sẵn tính năng sub-agent (Agent tool). Bạn không
  cần cài thêm gì. Chỉ cần config đúng model routing, rồi khi gõ "spawn agent sonnet"
  hoặc để Opus tự spawn, nó sẽ dùng model sonnet đã map trong settings.
</Callout>
...
- **Knowns** — dùng `appendNotes` để ghi kết quả vào task, hoặc tạo doc riêng cho mỗi phase
- **Markdown tự cấu trúc** — folder `docs/handoff/` hoặc bất kỳ convention nào team bạn quen

Ý tưởng đều giống nhau: có một chỗ ghi lại "phase trước làm xong gì, phát hiện gì, quyết định gì" để phase sau đọc được mà không cần hỏi lại.

<Callout type="tip">
  File trung gian không cần đẹp. Nó chỉ cần đủ thông tin để agent tiếp theo không
  phải hỏi lại hoặc đọc lại từ đầu. Nghĩ như bản bàn giao ca cho đồng nghiệp.
</Callout>
...
<Quote color="green">
  Đừng cố chấp vào một tool duy nhất. Hệ thống tốt là hệ thống giúp supervisor agent giữ
  context đúng, không phải hệ thống có tên nghe xịn nhất.
</Quote>
...
<Callout type="note">
  Workflow tốt không phải workflow phức tạp nhất. Workflow tốt là workflow vừa đủ
  để giảm lỗi và giảm chi phí cho loại task đang làm.
</Callout>
