...
<Callout type="warning">
  Đừng paste <TermNote id="secret">API key</TermNote> vào blog, repo, issue, hay prompt public. Config local nên
...
Đây là phần nhiều người không biết: **sub-agent là tính năng có sẵn trong Claude Code**. Bạn không cần cài plugin, không cần <TermNote id="mcp">MCP server</TermNote> riêng, không cần script wrapper.
...
- Opus không phải đọc hết mọi file - tiết kiệm <TermNote id="context-window">context window</TermNote>
...
- [Backlog.md](https://github.com/MrLesk/Backlog.md) - Hệ thống quản lý task bằng markdown. Nhẹ, dễ dùng với agent, không cần setup phức tạp.
- [Knowns](https://knowns.sh) - MCP-based project memory system. Quản lý task, docs, memory, và context cho agent trong một lớp.
...
Mỗi sub-agent chỉ đụng phần của mình. Opus không cần theo dõi chi tiết từng dòng code - chỉ cần đọc summary và review <TermNote id="diff">diff</TermNote>.
